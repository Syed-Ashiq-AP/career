...
export const Survey = () => {
      ...
      try {
        ...
        keywordsToFetch.forEach((keyword, index) => {
          if (data.images && data.images[index]) {
            ...
            const img = new Image();
            img.onload = () => {
              setLoadedImages((prev) => ({ ...prev, [optionKey]: true }));
            };
            img.onerror = () => {
            ...
            // Update image map
            setOptionImages((prev) => ({ ...prev, [optionKey]: imageUrl }));
          }
        });
      } catch (error) {
  ...
  const handleAnswer = async (optionKey: string) => {
    ...
    if (isChangingAnswer) {
      ...
      const newQuestionHistory = questionHistory.slice(0, questionIndex + 1);
  ...
  if (error) {
    return (
      <div className="bg-card text-card-foreground flex flex-col gap-4 rounded-xl border p-4 shadow-sm sm:w-md">
        ...
        <button onClick={startSurvey} className="text-primary underline">
          Retry
        </button>
      </div>
    );
