...
export default function EnhancedChatBot({
    ...
    const handleConversationIdChange = useCallback(
        (conversationId: string) => {
            // Only update URL if we don't already have a conversation ID in the URL
            if (!initialConversationId) {
                // Use History API to update URL without navigation
                const newUrl = `/ai-enhanced/${conversationId}`;
    ...
    const { messages, isStreaming, isLoading, error, send, cancel } = useChat({
        userId,
        initialConversationId,
        onError: (error) => {
    ...
    const commandSuggestions: CommandSuggestion[] = useMemo(
        () => [
            ...
            },
            {
                ...
                label: "Progress",
                description:
                    "Displays the user's application or learning status",
                prefix: "/progress",
            },
            {
                ...
                label: "Resume",
                description:
                    "Analyze or generate resume/CV for B.Tech admissions",
                prefix: "/resume",
            },
            {
    ...
    const handleSendMessage = async () => {
        if (value.trim() && !isStreaming && !isLoading) {
            const messageToSend = value;
            setValue("");
            adjustHeight(true);

            try {
                await send({ message: messageToSend, stream: true });
            } catch (error) {
                ...
                <AnimatePresence>
                    {error && (
                        <motion.div
                            ...
                            exit={{ opacity: 0, y: -10 }}
                            className="absolute top-4 right-4 bg-red-500/10 border border-red-500/20 text-red-600 px-4 py-2 rounded-lg shadow-lg backdrop-blur-sm"
                        >
